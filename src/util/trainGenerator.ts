...
export default function* () {
  ...
  const imageBin = fs.readFileSync(`data/emnist-letters-train-images`);
  const labelsBin = fs.readFileSync(`data/emnist-letters-train-labels`);
  ...
  const images: { image: number[][], label: tf.Tensor; }[] = new Array(numImages);
